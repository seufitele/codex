 ...
 *
 */
public class SeqCollectionComparator
{
	private SeqCollectionComparator()
	{
	...
	public static <T extends Comparable<? super T> , U, W> void testSharpCollectionComparator(final ComparableCollection<T> sharpCollection,
			final ComparableCollection<T> originalCollection, final ComparableCollection<T> emptyCollection,
			final Comparator<? super T> theComparator, final T eleNotInCollection, final T... elems)
	{
		...
		// min & minOption
		assertTrue(sharpCollection.min().get().equals(sortedSet.first()));
		assertTrue(emptyCollection.min().isEmpty());

		// minWith
		assertTrue(sharpCollection.min(theComparator).equals(ssComparator.first()));

		// max & maxOption
		assertTrue(sharpCollection.max().get().equals(sortedSet.last()));
		assertTrue(emptyCollection.max().isEmpty());
