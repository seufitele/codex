...
public abstract class State<S, A> implements Monad<A>, Function1<S, Tuple2<A, S>>
{
	...
	public <B> State<S, B> map(final Function1<? super A, B> function)
	{
		return new State<S, B>()
		{
			...
			{
				final Tuple2<A, S> res = State.this.apply(param);
				return Tuple2.from(function.apply(res.getVal1()), res.getVal2());
			}
		};
	}
	...
	@Override
	public <U> State<S, U> bind(final Function1<? super A, ? extends Monad<U>> function)
	{
		return new State<S, U>()
		{
			@SuppressWarnings("unchecked")
